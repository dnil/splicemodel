#!/usr/bin/perl -w -I/home/daniel/work/reggae -I.
...
if ($gff) { # GFF-output # longest/shortest here as well?
    map { print $gff2outfh $$_->gff_string."\n" } @ofeat;     # output all ok cds feats
...
if ($report) { # model results summary + gff partitioned into different categories # report == 1
	...
	map { print $reportoutfh $$_->gff_string."\n" } @dfeat;
...
sub write_gff_utrs_for_cds {
	...
	foreach my $sladdpos ( $$accepted_cds_feat->each_tag_value('SLadd') ) {
	    ...
	    if($strand == 1) {
		print GFF_UTR_OUT "$seq_name\tpapya_$version\t5pUTR\t$sladdpos\t".($cdsatgpos-1)."\t.\t$strandchar\t.\t$comment\n"; # don't include the initial and end A's on the "UTR"
	    } else {
		print GFF_UTR_OUT "$seq_name\tpapya_$version\t5pUTR\t".($cdsatgpos+1)."\t$sladdpos\t.\t$strandchar\t.\t$comment\n"; # don't include the initial and end A's on the "UTR"
	    }
	}
    }
...
sub extremes_clean_cds_5p {
	...
	foreach my $accepted_cds_feat ( @ofeat ) {
	    ...
	    $$shortest_ppy_feat->add_tag_value('SLaddForCDS',"This ppyag was implied as the trans-splicesite for the downstream CDS ".$$accepted_cds_feat->gff_string);
...
sub extremes_clean_cds_3p {	
	...
	foreach my $accepted_cds_feat ( @opafeat ) {
		...
		$$accepted_cds_feat->remove_tag('polya');
           ...
           # quick debug
#	    defined($accepted_cds_feat_end_pos) || print STDERR "undefined acc_cds_feat_end for ".$$accepted_cds_feat->gff_string."\n";
#	    defined($shortest_polya_pos) ||  print STDERR "undefined shortest polyapos for ".$$accepted_cds_feat->gff_string."\n";
...
sub output_uorfs {
	...
	if ($feat->primary_tag eq "uORF") {
	    print $uorfoutfh $feat->gff_string."\n";
	}
    }
}
...
#	}
     ...
     foreach my $accepted_cds_feat ( @ofeat ) {
  	...
  	foreach my $sladdpos ( $$accepted_cds_feat->each_tag_value('SLadd') ) {
	    ...
	    if( $strand > 0 ) {
		my $pyagatgdist = $cdsatgpos - $sladdpos -1 ;
		...
		if(defined($rout)) {
			...
			.$polyagdist."\t".$polypylen."\t".$pyagatgdist."\t".$$accepted_cds_feat->gff_string."\n";
		}
	    } else {
		my $pyagatgdist = ($sladdpos - $cdsatgpos - 1); 
...
#	 my ($tpUTRlen) = ($polyapos - $cdsendpos - 1); # distance between two inclusive coords..
	 ...
	 if(defined($rout)) {
		 ...
		 ."\t".$polyagdist."\t".$na."\t".$polypylen."\t".$tpUTRlen."\t".$accepted_cds_feat->gff_string."\n";
	 }
     }
 }
...
sub polypyag_cds_start_check {
    ...
    # $DEBUG && map { debug($_->gff_string) } @cfeat;
			...
			if($cfeat[$i]->strand == $direction && $cfeat[$i]->primary_tag eq "CDS") {
				...
				if($lastpolypyagend != 1) {
					...
					if(defined($rout)) {
						...
						.$polyagdist."\t".$polypylen."\t".($cfeat_start - $lastpolypyagend -1)."\t".$cfeat[$i]->gff_string."\n";
					}
				    }

				    $$lastpolypyfeat->add_tag_value('SLaddForCDS',"This ppyag was implied as the trans-splicesite for the downstream CDS ".$cfeat[$i]->gff_string);
				    $$lastpolypyfeat->add_tag_value('cds-atg',$cfeat_start);
				    
				    $cfeat[$i]->add_tag_value('Polypymodel',"Agreement: distance from 5p_ag to cds atg is ".($cfeat_start - $lastpolypyagend));
				...
				if($lastpolypyagend != $seqlen) {
				    ...
				    $cfeat[$i]->add_tag_value('Polypymodel',"Agreement: distance to nearest previous 5p_ag to cds atg is ".($lastpolypyagend - $cfeat_end));
					...
					if(defined($rout)) {
					    ...
					    print $statisticsfh "ppyag\t".$$lastpolypyfeat->start."\t".$$lastpolypyfeat->end."\t".$cfeat_start."\t".$cfeat_end."\t"
						.$polyagdist."\t".$polypylen."\t".($lastpolypyagend - $cfeat_end -1)."\t".$cfeat[$i]->gff_string."\n";
					}
				    }
	...
	if(${$cds_feat[$i]}->strand == 1) {
		...
		if(!($long || $short) && defined($statisticsfile)) { # type, start, stop, pyagdist, polypylen, polya-pyag
			...
			print $statisticsfh "pappy\t".${$pyag_feat[$j]}->start."\t".${$pyag_feat[$j]}->end."\t".$cds_feat_start."\t".$cds_feat_end."\t".$polyagdist
			    ."\t".$polypylen."\t".$tpUTRlen."\t".${$cds_feat[$i]}->gff_string."\n";
		    }
		}
...
# 						."\t".$polypylen."\t".$tpUTRlen."\t".$cfeat[$i]->gff_string."\n";
# 					}
# 				    }
...
# 					    # .$polya_pyag_dist."), leaving a negatively sized 3'UTR for ".$cfeat[$i]->gff_string);
# 					#    next OTHERFEAT;
# 					# }
# 					if( ($possible_polyasite > $cfeat_start)
# 					    || ($possible_polyasite < $lastpolypyagstart) ) { 
